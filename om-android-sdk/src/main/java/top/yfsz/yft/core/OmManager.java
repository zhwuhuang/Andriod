...
import top.yfsz.yft.core.imp.interstitialad.IsManager;
import top.yfsz.yft.core.imp.rewardedvideo.RvManager;
...
import top.yfsz.yft.utils.helper.IapHelper;
...
import static top.yfsz.yft.OmAds.AD_TYPE;
...
public final class OmManager implements InitCallback {
    ...
    public boolean isInterstitialAdReady(String placementId) {
        IsManager isManager = getIsManager(placementId);
        if (isManager != null) {
            boolean result = isManager.isInterstitialAdReady();//dian
            if (result) {
